...
function parseMessage(content: string) {
  const parts: Array<{ type: "text"; text: string } | { type: "campaign"; id: string; title: string }> = []
...
export default function AIPage() {
  const [messages, setMessages] = useState<Message[]>([
    { role: "assistant", content: "Assalamualaikum! 👋 Saya Donyar AI, siap membantu kamu menemukan campaign donasi terbaik. Mau donasi untuk apa hari ini? 🌿" }
  ])
  ...
  return (
    <main className="flex flex-col h-screen" style={{ background: "#f0f7f0" }}>
      ...
      {/* HEADER */}
      <div className="px-5 pt-12 pb-4 flex items-center gap-3 rounded-b-4xl shadow-md"
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4 flex flex-col gap-3">
        {messages.map((m, i) => (
          <div key={i} className={`flex gap-2 ${m.role === "user" ? "justify-end" : "justify-start"}`}>
            {m.role === "assistant" && (
              <div className="w-8 h-8 rounded-xl bg-green-100 flex items-center justify-center text-sm shrink-0 mt-0.5">🤖</div>
            )}
            <div className={`max-w-[82%] flex flex-col gap-2`}>
              {/* Bubble teks */}
              {parseMessage(m.content).map((part, j) =>
                part.type === "text" && part.text.trim() ? (
                  <div key={j} className={`px-4 py-3 rounded-3xl text-sm leading-relaxed ${
                    m.role === "user"
                      ? "text-white rounded-tr-none"
                      : "bg-white text-gray-700 shadow-sm rounded-tl-none"
                  }`}
                  style={m.role === "user" ? { background: "linear-gradient(135deg, #22c55e, #16a34a)" } : {}}>
                    {part.text}
                  </div>
                ) : part.type === "campaign" ? (
                  ...
                  <Link key={j} href={`/campaign/${part.id}`}>
                    <div className="bg-white border border-green-200 rounded-2xl p-3 shadow-sm flex items-center gap-3 active:scale-[0.98] transition-all">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-bold text-gray-800 text-sm">{part.title}</p>
                        <p className="text-green-600 text-xs mt-0.5">Tap untuk donasi →</p>
                      </div>
                    </div>
                  </Link>
                ) : null
        ...
        {loading && (
          <div className="flex gap-2 justify-start">
            <div className="w-8 h-8 rounded-xl bg-green-100 flex items-center justify-center text-sm shrink-0">🤖</div>
