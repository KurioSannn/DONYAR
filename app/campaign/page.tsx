...
export default function CampaignPage() {
  ...
  return (
    <main className="min-h-screen pb-24" style={{ background: "#f0f7f0" }}>
      ...
      {/* HEADER */}
      <div className="px-5 pt-12 pb-8 rounded-b-[40px]"
        style={{ background: "linear-gradient(160deg, #1a6b1c 0%, #16a34a 100%)" }}>
        <span className="bg-white/20 text-white text-xs font-bold px-3 py-1 rounded-full">🌙 RAMADHAN 1447H</span>
        <h1 className="text-white font-bold text-2xl mt-2">Campaign Donasi</h1>
        <p className="text-green-200 text-sm mt-0.5">Pilih campaign yang ingin kamu dukung</p>
      ...
      <div className="px-5 mt-5">
        {loading ? (
          ...
          <div className="bg-white rounded-3xl p-10 text-center shadow-sm mt-4">
            ...
            <p className="text-gray-400 text-sm">Tidak ada campaign ditemukan</p>
          </div>
        ) : (
                  ...
                  <div className="bg-white rounded-3xl p-4 shadow-sm active:scale-[0.98] transition-all">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-gray-400 text-xs mt-0.5 line-clamp-2">{c.description}</p>

                        <div className="mt-3">
                          <div className="flex justify-between text-xs mb-1">
                            <span className="text-green-600 font-bold">Rp {c.collectedAmount.toLocaleString("id-ID")}</span>
                            <span className="bg-green-50 text-green-600 px-2 py-0.5 rounded-full font-bold">{pct}%</span>
                          </div>
                          ...
                          <div className="flex justify-between items-center mt-1.5">
                            <p className="text-gray-400 text-xs">Target: Rp {c.targetAmount.toLocaleString("id-ID")}</p>
                            <span className="text-green-600 text-xs font-bold">Donasi →</span>
                          </div>
                        </div>
                      </div>
                    </div>
