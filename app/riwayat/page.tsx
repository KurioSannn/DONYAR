...
export default function RiwayatPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/donation?userId=${session.user.id}`)
      .then(r => r.json()).then(data => { setDonations(data); setLoading(false) })
  }, [status, session])
  ...
  return (
    <main className="min-h-screen pb-24" style={{ background: "#f0f7f0" }}>
      ...
      <div className="px-5 pt-12 pb-8 rounded-b-[40px]"
        ...
        <span className="bg-white/20 text-white text-xs font-bold px-3 py-1 rounded-full">📜 RIWAYAT</span>
        <h1 className="text-white font-bold text-2xl mt-2">Riwayat Donasi</h1>
        <p className="text-green-200 text-sm mt-0.5">Semua donasi yang telah dilakukan</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 px-5 -mt-5">
        ...
        <div className="bg-white rounded-3xl p-4 shadow-md text-center">
          <p className="text-base font-bold text-yellow-500">Rp {total.toLocaleString("id-ID")}</p>
